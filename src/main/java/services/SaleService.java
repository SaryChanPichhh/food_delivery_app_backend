...
import com.group_one.food_delivery_app.models.*;
import com.group_one.food_delivery_app.repositories.*;
...
public class SaleService implements ISaleService {
    ...
    public void addToCart(int userId, int menuId, int resId) {
        ...
        var defaultRateOpt = _exchangeRateRepository.findByIsDefaultTrue();
        ...
        SaleDetailModel detail = _cartRepository.findBySaleHeader_IdAndItemCode(cart.getId(), String.valueOf(menu.getId()));
    ...
    public void updateCartItemQuantity(int userId, int detailId, int changeAmount) {
        ...
        if (detail == null || detail.getSaleHeader().getId() != cart.getId()) return;
        ...
        if (newCartTotal < 0.01) newCartTotal = 0.0;
    ...
    @Override
    @Transactional
    public void checkoutCart(int userId, com.group_one.food_delivery_app.utils.enums.PaymentMethod paymentMethod, Long deliveryId) {
        SaleHeaderModel cart = getActiveCart(userId);
        if (cart != null) {
    ...
    public void checkoutCart(int userId, Long deliveryId) {
        checkoutCart(userId, com.group_one.food_delivery_app.utils.enums.PaymentMethod.CASH, deliveryId);
    }
}
